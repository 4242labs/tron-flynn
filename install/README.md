...
3. **Telegram** (per machine, optional): `tg-send.sh` reads `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` from `.env` at this repo's root (gitignored). No `.env` and TG wanted → Boot asks for the two values. Message formats live in `skills/skill-operator-comms.md`.
