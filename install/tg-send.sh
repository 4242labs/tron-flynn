#!/usr/bin/env bash
# tg-send.sh — send one line to the operator's Telegram chat. Ported from tron-app v1.
# Reads TELEGRAM_BOT_TOKEN + TELEGRAM_CHAT_ID from the agent's .env (repo root, gitignored).
#   tg-send.sh "<message>"
set -euo pipefail
